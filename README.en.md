...
1. Create a directory for the project that you want to create (example:
   `sivax_b08`), then open a Command Prompt (cmd) or Terminal in
   that directory.
...
5. Create a new Django project with this template.
   ...
   ```shell
   django-admin startproject --template="https://codeload.github.com/laymonage/django-template-heroku/zip/template" --extension="py,yml,md" --name="Procfile" sivax_b08 .
   ```

   > Note: rename `sivax_b08` with the desired project name. Notice
...
3. Set the code coverage configuration on GitLab.
   ...
   command.
    ...
    ```shell
    coverage run --include="./*" --omit="venv/*,manage.py,sivax_b08/*" manage.py test
    ```
